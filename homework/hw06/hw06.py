    ...
    def vend(self):
        if self.number <= 0:
           return 'Nothing left to vend. Please restock.'
            ...
            div = self.price - self.balance
            return f'Please add ${div} more funds.'
        elif self.balance > self.price:
            div = self.balance - self.price
            self.balance = 0
            self.number -= 1
            return f'Here is your {self.product} and ${div} change.'
        elif self.balance == self.price:
            self.number -= 1
            self.balance = 0
...
def two_list(vals, counts):
    ...
    def helper(index, count):
        if count == 0:
            if len(vals) == index + 1:
                return Link.empty
            return Link(vals[index + 1], helper(index + 1, counts[index + 1] - 1))
