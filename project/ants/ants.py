...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        # target_p = lambda target, place: place if target == 0 else target_p(target - 1, place.entrance)
        # start_p = target_p(self.lower_bound, self.place)
        # place = start_p
        # step = self.upper_bound - self.lower_bound + 1
        # while step:
        #     if place.is_hive:
        #         return None
        #     if place.bees:
        ...
        #     place = place.entrance
        #     step -= 1
        # Error Problem 4: place is None
        step = 0
        now_place = self.place
        while True:
            if now_place.is_hive:
                return None
            if len(now_place.bees) == 0 or step < self.lower_bound or step > self.upper_bound:
    ...
    def action(self, gamestate):
        ...
        while behind is not None:
            ...
            if ant is not None:
                if ant.is_container:
                    ant.double()
                    ant_i = ant.ant_contained
                    if ant_i is not FireAnt and ant_i is not None:
                        ant_i.double()
                else:
...
class NinjaAnt(Ant):
    ...
    def action(self, gamestate):
        # BEGIN Problem Optional 1
        "*** YOUR CODE HERE ***"
        bees = list(self.place.bees)                    #trans to iterable  
        for bee in bees:                                #traverse lead error:list index out of range
            bee.reduce_health(self.damage)              
        # END Problem Optional 1
