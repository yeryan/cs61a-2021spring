...
def more_boar(player_score, opponent_score):
    ...
    # BEGIN PROBLEM 4
    "*** YOUR CODE HERE ***"
    play = str(player_score).zfill(2) if player_score < 10 else str(player_score)
    oppo = str(opponent_score).zfill(2) if opponent_score < 10 else str(opponent_score)
    pla = list(play)
    opp = list(oppo)
    return pla[0] < opp[0] and pla[1] < opp[1]
...
def max_scoring_num_rolls(dice=six_sided, trials_count=1000):
    ...
    '''    avg = dict()
    for i in range(1, 11):
        get_avg = make_averaged(roll_dice, trials_count)
        cur_avg = int(get_avg(i, dice))
        if cur_avg not in avg:
            avg[cur_avg] = i
        else:
            if i < avg[cur_avg]:
                avg[cur_avg] = i
