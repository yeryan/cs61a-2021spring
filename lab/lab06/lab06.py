    ...
    def report(self):
        ...
        if self.changed():
            ...
            if self.before < self.after:
                return '{0}: increased {1}->{2}' .format(self.id, self.before, self.after)
            return '{0}: decreased {1}->{2}' .format(self.id, self.before, self.after)
...
class ChangeMachine:
    ...
    def change(self, coin):
        ...
        "*** YOUR CODE HERE ***"
        self.coins[coin] = 1 + self.coins.get(coin, 0)  # Put the coin in the machine 返回指定面额硬币的数量，若没有该硬币，返回0
